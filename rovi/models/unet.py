...
class RoviModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if self.mask_decoder is not None:
            ...
            mask = self.mask_decoder(cross_attn_up_features)
        ...
        return UNet3DConditionOutput(sample=sample, mask=mask)

    @classmethod
    def from_pretrained_2d(cls, pretrained_model_path, user_config, subfolder=None):
        ...
        config["temp_attn"] = user_config.get("temp_attn", True)
        config["ref_frame_type_2"] = user_config.get("ref_frame_type_2", None)
        config["cross_frame_type"] = user_config.get("cross_frame_type", None)
        config["mask_decoder_type"] = user_config.get("mask_decoder_type", None)
        config["lan_cross_attn"] = user_config.get("lan_cross_attn", False)

        from diffusers.utils import WEIGHTS_NAME
        model = cls.from_config(config)
        if user_config.load_weight:
            ...
            msg = "Do not load unet pre-trianed weights."
